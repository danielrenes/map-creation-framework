...
class TestHierarchical(NoLoggingTestCase):
    def setUp(self):
        path_1 = Path()
    ...
    def test_single_linkage(self):
        dist = self.algorithm.single_linkage(
            [self.paths[0], self.paths[2], self.paths[4]],
            [self.paths[1], self.paths[3], self.paths[5]])
        self.assertAlmostEqual(dist, 0.1730, 4)

    def test_complete_linkage(self):
        dist = self.algorithm.complete_linkage(
            [self.paths[0], self.paths[2], self.paths[4]],
            [self.paths[1], self.paths[3], self.paths[5]])
        self.assertAlmostEqual(dist, 0.8326, 4)

    def test_average_linkage(self):
        dist = self.algorithm.average_linkage(
            [self.paths[0], self.paths[2], self.paths[4]],
            [self.paths[1], self.paths[3], self.paths[5]])
        self.assertAlmostEqual(dist, 0.4287, 4)
